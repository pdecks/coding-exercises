...
class FriendGraph(object):
    ...
    def are_connected(self, name1, name2):
        ...
        friend = self.nodes[name2]
        visited.append(person)

        if person.adjacent:
            to_visit.extend(person.adjacent)
        else:
            return False
        # visit adjacent friends
        while to_visit:
            current_friend = to_visit.popleft()
            if current_friend not in visited:
                visited.append(current_friend)
                if current_friend == friend:
                    return True
                # if other unvisited adjacent friends, add to queue, repeat
                elif current_friend.adjacent:
                    to_visit.extend(current_friend.adjacent)
        return False
